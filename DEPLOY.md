...
How to deploy ParkinsonDiet to [fly.io](https://fly.io) as a single container, run it in production, and verify it. Nothing here is created automatically — the deployment artifacts live in `scripts/` (see below) and you deploy with `scripts/deploy.sh`.

> **A reference deployment is already live** at `https://ParkinsonDiet-ed.fly.dev` (custom domain `https://ParkinsonDiet.edwarddonner.com`). The identifiers below — app `ParkinsonDiet-ed`, region `sjc`, domain `ParkinsonDiet.edwarddonner.com` — are that owner's. **To stand up your own**, pick a globally-unique Fly app name and a region near your Supabase DB, then change them in two places: `APP="..."` in `scripts/deploy.sh` and `app = "..."` / `primary_region = "..."` in `scripts/fly.toml` (keep the two app names in sync). Read every `-a ParkinsonDiet-ed`, `ParkinsonDiet-ed.fly.dev`, and `*.edwarddonner.com` below as `<your-app>` / `<your-domain>` placeholders.

| | |
|---|---|
| **App** | `ParkinsonDiet-ed` → `https://ParkinsonDiet-ed.fly.dev` |
| **Region** | `sjc` (San Jose) |
...
```toml
# Fly.io config for the ParkinsonDiet app. Deploy with scripts/deploy.sh.
app = "ParkinsonDiet-ed"
...
```bash
#!/usr/bin/env bash
# Build and deploy ParkinsonDiet to fly.io: create the app on first run, stage secrets
...
- Secrets can be set/changed any time: `fly secrets set -a ParkinsonDiet-ed KEY=value` (triggers a rolling restart). View names with `fly secrets list -a ParkinsonDiet-ed` (values are never shown).
...
```bash
fly scale count 2 -a ParkinsonDiet-ed     # min_machines_running=1 keeps 1 warm; soft_limit balances across both
```
...
Run against `https://ParkinsonDiet-ed.fly.dev`. Use `MODEL=openai/gpt-5.4-nano` for cheap test calls if you like, and clean up test data afterwards.

- [ ] `fly status -a ParkinsonDiet-ed` — 1 machine in `sjc`, state `started`, health check **passing**.
- [ ] `curl -s https://ParkinsonDiet-ed.fly.dev/api/config` → `{"owner_name":"..."}` (200).
- [ ] `/` loads the visitor UI (dark + light, desktop + mobile); the rings background and the LinkedIn/YouTube footer render.
- [ ] A normal question streams a reply (real LLM call); `Q2` returns the instant FAQ; `https://ParkinsonDiet-ed.fly.dev/?q=2` opens and immediately answers Q2.
...
- [ ] `fly logs -a ParkinsonDiet-ed` shows no errors during the above.
...
- The app is reachable at `https://ParkinsonDiet-ed.fly.dev` with HTTPS forced, ≥1 machine always running in `sjc`, and the health check green.
...
Mapping the app to your own domain is optional — `https://<your-app>.fly.dev` works on its own. A subdomain of your site is worth it mainly for clean **iframe embedding**: serving the app from `ParkinsonDiet.<yourdomain>` (the same registrable domain as the host page) keeps the "Keep chat" cookie **first-party**, avoiding third-party-cookie blocking.

1. Request the certificate:

   ```bash
   fly certs add ParkinsonDiet.<yourdomain> -a <your-app>
   ```

2. Add a **CNAME** record at your DNS provider, pointing `ParkinsonDiet` at the Fly hashed target shown by `fly certs show ParkinsonDiet.<yourdomain> -a <your-app>` (e.g. `pq9wl1k.<your-app>.fly.dev`). Prefer this CNAME over the A/AAAA records Fly also lists: the app's IPv4 is a *shared* Fly address (not exclusively yours), and a CNAME automatically tracks any Fly IP change. Use a short TTL (~300s) during setup so corrections propagate quickly.
...
3. Watch issuance with `fly certs check ParkinsonDiet.<yourdomain> -a <your-app>`. Once DNS propagates, Fly issues the cert automatically and the app is served at `https://ParkinsonDiet.<yourdomain>` over HTTPS.

**Embedding.** A ready-to-paste snippet is in `scripts/wordpress-embed.html` (a WordPress "Custom HTML" block): it pins the app full-bleed just below the site nav, overrides the theme's content-column max-width, guards against horizontal overflow on narrow screens, and forwards `?q=N` from the host page into the iframe. Change two values for your own site — the `BASE` constant (your subdomain, e.g. `https://ParkinsonDiet.<yourdomain>`) and the iframe `title` (your own name). See SPEC.md "Tech stack decisions" for the `frame-ancestors` guidance.

## 8. Operations

- Status / logs: `fly status -a ParkinsonDiet-ed`, `fly logs -a ParkinsonDiet-ed`.
- Scale up/out: `fly scale vm shared-cpu-4x -a ParkinsonDiet-ed` (bigger), `fly scale count 2 -a ParkinsonDiet-ed` (more machines).
- Roll back: `fly releases -a ParkinsonDiet-ed` then `fly deploy` a prior image, or `fly releases rollback -a ParkinsonDiet-ed`.
- Secrets change: `fly secrets set -a ParkinsonDiet-ed KEY=value` (rolling restart).
